...
@section('title', 'Tag: #' . $tag->name . ' - SaaSNinja Blog')
...
@section('content')
    <header class="bg-slate-50 dark:bg-slate-950 py-16 border-b border-slate-200 dark:border-slate-800">
        <div class="max-w-7xl mx-auto px-6">
            ...
            <h1 class="font-outfit font-extrabold text-4xl text-slate-900 dark:text-white mb-2">#{{ $tag->name }}</h1>
            <p class="text-slate-500 text-sm max-w-xl">Browse all publications tagged with #{{ $tag->slug }} index reference.</p>
        </div>
    </header>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-2 space-y-8">
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        ...
                        <article class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-shadow flex flex-col justify-between group">
                            ...
                            <div class="p-5 pt-0 border-t border-slate-50 dark:border-slate-800/80 flex items-center justify-between mt-4">
                                <div class="text-[10px] text-slate-400 space-y-0.5">
                                    <div class="font-semibold text-slate-655 dark:text-slate-355">By <a href="{{ route('blog.author', $post->author->id) }}" class="hover:underline text-slate-800 dark:text-white">{{ $post->author->name }}</a></div>
                    ...
                    @empty
                        <div class="col-span-2 text-center py-12 text-slate-500 italic">No publications created under this tag yet.</div>
                    @endforelse
                </div>
            ...
            <!-- Right Side: Sidebar tags -->
            <div class="space-y-6">
                <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-5 rounded-2xl shadow-sm space-y-4">
                    <h4 class="font-outfit font-bold text-xs uppercase tracking-wider text-slate-900 dark:text-white">All Tags</h4>
                    <div class="flex flex-wrap gap-2 text-xs">
                        @foreach($tags as $t)
                            <a href="{{ route('blogTag', $t->slug) }}" class="px-2.5 py-1 {{ $tag->id === $t->id ? 'bg-primary text-white' : 'bg-slate-50 dark:bg-slate-950 text-slate-600 dark:text-slate-400 hover:bg-primary hover:text-white' }} border border-slate-200/60 dark:border-slate-800 rounded-lg transition-colors font-semibold">
                                #{{ $t->name }}
                            </a>
                        @endforeach
                    </div>
