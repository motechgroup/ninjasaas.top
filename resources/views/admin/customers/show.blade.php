...
<div class="space-y-6">
    ...
    <div class="bg-white dark:bg-slate-900 rounded-xl p-6 border border-slate-200/80 dark:border-slate-800 shadow-sm flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
        ...
        @if($customer->id !== auth()->id())
            <div class="flex flex-wrap gap-3">
                <!-- Status Control Modal Triggers -->
                <button type="button" onclick="document.getElementById('statusModal').classList.remove('hidden')" class="px-4 py-2 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 text-slate-700 dark:text-slate-200 text-xs font-bold rounded-lg transition-colors">
                    Change Account Status
                </button>

                <form method="POST" action="{{ route('admin.customers.destroy', $customer) }}" onsubmit="return confirm('Are you sure you want to permanently delete customer account {{ $customer->name }}? This action cannot be undone.')" class="inline">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="px-4 py-2 bg-rose-600 hover:bg-rose-700 text-white text-xs font-bold rounded-lg transition-colors">
                        Delete Account
                    </button>
                </form>
            </div>
    ...
    <!-- Status Change Modal -->
    <div id="statusModal" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
        <div class="bg-white dark:bg-slate-900 rounded-xl max-w-md w-full p-6 border border-slate-200/80 dark:border-slate-800 shadow-xl">
            <h3 class="text-lg font-bold text-slate-900 dark:text-white font-outfit mb-4">Update Account Status</h3>
            <form method="POST" action="{{ route('admin.customers.status', $customer) }}" class="space-y-4">
                ...
                <div class="flex justify-end space-x-3 pt-4 border-t border-slate-100 dark:border-slate-800">
                    <button type="button" onclick="document.getElementById('statusModal').classList.add('hidden')" class="px-4 py-2 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 text-xs font-semibold rounded-lg hover:bg-slate-200 transition-colors">
                        Cancel
                    </button>
