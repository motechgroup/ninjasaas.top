<x-app-layout>
    ...
    <div x-data="{
        ...
        <form action="{{ route('admin.blog.store') }}" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            @csrf
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 rounded-2xl shadow-sm space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="title" x-model="title" @input="generateSlug()" required class="block w-full rounded-lg border-slate-200 dark:border-slate-800 dark:bg-slate-950 text-slate-900 dark:text-white text-sm py-2.5 px-3">
                    </div>

                    <!-- Subtitle -->
                    <div>
                        <label class="block text-xs font-bold text-slate-700 dark:text-slate-300 uppercase mb-1">Subtitle / Excerpt Banner</label>
                        <input type="text" name="subtitle" x-model="subtitle" class="block w-full rounded-lg border-slate-200 dark:border-slate-800 dark:bg-slate-955 text-slate-900 dark:text-white text-sm py-2.5 px-3">
                    </div>
                ...
                <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 rounded-2xl shadow-sm space-y-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div class="space-y-1">
                            ...
                            <textarea name="content" x-model="content" required rows="16" class="block w-full font-mono rounded-lg border-slate-200 dark:border-slate-800 dark:bg-slate-950 text-slate-900 dark:text-white text-xs py-2 px-3" placeholder="# Heading 1&#10;Write content in Markdown..."></textarea>
                        </div>
                        <!-- Live preview box -->
                        <div class="space-y-1">
                            ...
                            <div class="border border-slate-200 dark:border-slate-800 dark:bg-slate-950 rounded-lg p-3 min-h-[300px] h-[340px] overflow-y-auto prose prose-xs dark:prose-invert">
                                <div class="text-slate-650 dark:text-slate-400 text-xs whitespace-pre-wrap" x-text="content || 'Editor preview will compile here...' "></div>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 rounded-2xl shadow-sm space-y-4">
                    ...
                    <div class="space-y-3">
                        <template x-for="(faq, index) in faqs" :key="index">
                            <div class="p-4 bg-slate-50 dark:bg-slate-950 rounded-xl space-y-3 relative">
            ...
            <div class="space-y-6">
                ...
                <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 rounded-2xl shadow-sm space-y-4">
                    ...
                    <div>
                        ...
                        <input type="datetime-local" name="published_at" x-model="publishedAt" class="block w-full rounded-lg border-slate-200 dark:border-slate-800 dark:bg-slate-955 text-slate-900 dark:text-white text-sm py-2 px-3">
                    ...
                    <div>
                        ...
                        <select name="is_featured" x-model="isFeatured" required class="block w-full rounded-lg border-slate-200 dark:border-slate-800 dark:bg-slate-950 text-slate-900 dark:text-white text-sm py-2 px-3">
                    ...
                    <div>
                        ...
                        <input type="file" name="image" accept="image/*" class="block w-full rounded-lg border border-slate-200 dark:border-slate-800 dark:bg-slate-955 text-slate-900 dark:text-white text-xs py-2 px-3">
                    </div>
                        ...
                        <div class="space-y-2 border border-slate-200 dark:border-slate-800 rounded-lg p-3 max-h-40 overflow-y-auto bg-slate-50 dark:bg-slate-950">
                            @foreach($categories as $cat)
                                <label class="flex items-center gap-2 text-xs text-slate-700 dark:text-slate-300 cursor-pointer">
                                    <input type="checkbox" name="categories[]" value="{{ $cat->id }}" class="rounded text-primary focus:ring-primary">
                                    {{ $cat->name }}
                                </label>
                            @endforeach
                        </div>
                    ...
                    <div>
                        ...
                        <div class="space-y-2 border border-slate-200 dark:border-slate-800 rounded-lg p-3 max-h-40 overflow-y-auto bg-slate-50 dark:bg-slate-950">
                            @foreach($tags as $tag)
                                <label class="flex items-center gap-2 text-xs text-slate-700 dark:text-slate-300 cursor-pointer">
                                    <input type="checkbox" name="tags[]" value="{{ $tag->id }}" class="rounded text-primary focus:ring-primary">
                                    #{{ $tag->name }}
                                </label>
                            @endforeach
                        </div>
                ...
                <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 rounded-2xl shadow-sm space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="seo_title" x-model="seoTitle" class="block w-full rounded-lg border-slate-200 dark:border-slate-800 dark:bg-slate-955 text-slate-900 dark:text-white text-sm py-2 px-3" placeholder="If empty, matches title...">
                    </div>

                    <!-- SEO Description -->
                    <div>
                        <label class="block text-xs font-bold text-slate-700 dark:text-slate-300 uppercase mb-1">Meta Description</label>
                        <textarea name="seo_description" x-model="seoDescription" rows="3" class="block w-full rounded-lg border-slate-200 dark:border-slate-800 dark:bg-slate-950 text-slate-900 dark:text-white text-sm py-2 px-3" placeholder="Type Meta Description (Ideal length: 120-160 characters)..."></textarea>
